...
int find_name(const char *name) {
    ...
    for(i = 0; i < users[i].username[0] != ' '; i++) {
...
int main() {
    ...
    while(1){
        ...
        // create a subprocess to handle connection request just accept 
        if ((pid = fork()) == 0) {
            while(1){
                ...
                if (flag_recv == USERNAME) {
                    ...
                    printf("findName number: %d", name_num);
                    switch (name_num)
                    {
                        case -1:
                            send_data(conn_fd, "n\n");
